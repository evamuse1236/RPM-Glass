        ...
        try{
            if(args.getString("companion").equals("configure")){
                ...
                File store=CompanionStore.file(c);try{
                    ...
                    CompanionControls.apply(c,new JSONObject().put("action","widget_text_scale").put("value",160));check(CompanionControls.read(c).getInt("widgetTextScale")==160,"widget text size persisted through shared native control");
                    double systemScale=c.getResources().getConfiguration().fontScale;check(Math.abs(CompanionAlerts.status(c,true).getDouble("effectiveFontScale")-systemScale*1.6)<0.001,"capture status combines widget and Android font scales");check(Math.abs(CompanionAlerts.status(c,false).getDouble("effectiveFontScale")-systemScale)<0.001,"planner status keeps Android system font scale");
                    ...
                    try{CompanionControls.apply(c,new JSONObject().put("action","widget_text_scale").put("value",79));throw new AssertionError("invalid widget text size accepted");}catch(IllegalArgumentException expected){check(CompanionControls.read(c).getInt("widgetTextScale")==160,"invalid widget text size changes nothing");}
                    ...
                    check(CompanionSettingsController.validate(new JSONObject().put("action","choose_alarm")).equals("choose_alarm"),"settings action allowlist accepts supported native flow");
                    try{CompanionSettingsController.validate(new JSONObject().put("action","choose_alarm").put("path","/private"));throw new AssertionError("extra settings payload accepted");}catch(IllegalArgumentException expected){check(true,"settings action rejects unneeded values and paths");}
                    try{CompanionSettingsController.validate(new JSONObject().put("action","shell"));throw new AssertionError("arbitrary settings action accepted");}catch(IllegalArgumentException expected){check(true,"settings action allowlist rejects arbitrary native work");}
                    ...
                    JSONObject imported=fresh().put("intentV2",new JSONObject().put("captures",new JSONObject().put("capture",new JSONObject().put("raw","Synthetic source remains exact."))).put("transactions",new JSONObject().put("old-action",new JSONObject())).put("drafts",new JSONObject().put("draft",new JSONObject().put("status","review").put("revision",4).put("review",new JSONObject().put("token","old")))).put("sortPreviews",new JSONObject().put("sort",new JSONObject().put("status","preview").put("revision",1))));
                    CompanionStore.parkImportedDrafts(imported);JSONObject pilot=imported.getJSONObject("intentV2");
                    check(pilot.getJSONObject("captures").getJSONObject("capture").getString("raw").equals("Synthetic source remains exact."),"import preserves Glass capture source");
                    check(pilot.getJSONObject("transactions").length()==0,"import invalidates old action receipts");
                    JSONObject parked=pilot.getJSONObject("drafts").getJSONObject("draft");check(parked.getString("status").equals("parked")&&parked.getInt("revision")==5&&parked.isNull("review"),"import requires fresh draft approval");
                    check(pilot.getJSONObject("sortPreviews").getJSONObject("sort").getString("status").equals("dismissed"),"import cannot reapply a sort preview");
                    try{CompanionStore.validate(fresh().put("intentV2",new JSONObject().put("schema",2)));throw new AssertionError("unsupported capture format accepted");}catch(JSONException expected){check(true,"unsupported capture import rejected");}
                    try{CompanionStore.validate(fresh().put("intentV2",new JSONObject().put("schema",1).put("captures",new JSONArray())));throw new AssertionError("malformed capture format accepted");}catch(JSONException expected){check(true,"malformed capture import rejected");}
                }finally{CompanionAlerts.cancelAll(c);c.getSharedPreferences("companion-delivery",0).edit().clear().commit();c.getSharedPreferences("companion-appearance",0).edit().clear().commit();for(File f:c.getFilesDir().listFiles())if(f.getName().equals("companion.json")||f.getName().startsWith("companion-before-import-"))f.delete();}
            }
