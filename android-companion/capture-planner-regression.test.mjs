...
test('android respond binds a goal suggestion to the exact current message',async()=>{
 const data=freshStore(),raw='I want to learn more about cognitive load and context shifting and its effects, and see how Twitter, WhatsApp, and the missing deep focus hours are affecting me.';
 let request;
 const agent=createCompanionAgent({apiKey:'fixture',platform:'android',fetchImpl:async(_,options)=>{request=JSON.parse(options.body);return {ok:true,json:async()=>({model:MODEL,choices:[{finish_reason:'tool_calls',message:{tool_calls:[{id:'goal',type:'function',function:{name:'respond',arguments:JSON.stringify({message:'That could be held as a goal.',suggestions:[{label:'A goal?',text:'Turn this thought into a goal.',action:'goal_draft',title:'Understand how context shifting affects my deep focus'}]})}}]}}]})};}});
 const result=await agent(data,raw,{conversationId:data.conversations[0].id,now:new Date(2026,8,16)});
 const schema=request.tools.find(t=>t.function.name==='respond').function.parameters.properties.suggestions.items;
 assert.deepEqual(schema.required,['label','text']);assert.ok(schema.properties.action.enum.includes('goal_draft'));
 assert.equal(result.suggestions[0].captureAction.draft.sourceRaw,raw);assert.equal(result.suggestions[0].captureAction.draft.title,'Understand how context shifting affects my deep focus');assert.equal(data.planner,undefined);
});
