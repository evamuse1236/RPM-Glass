...
final class CompanionAlerts {
    ...
    static JSONObject status(Context c)throws JSONException{return status(c,false);}
    static JSONObject status(Context c,boolean widget)throws JSONException{channels(c);float systemScale=c.getResources().getConfiguration().fontScale,effectiveScale=systemScale*(widget?CompanionControls.widgetTextScale(c)/100f:1f);JSONObject out=new JSONObject().put("fontScale",systemScale).put("effectiveFontScale",effectiveScale).put("widgetTextScale",CompanionControls.widgetTextScale(c)).put("hasKey",CompanionKey.has(c)).put("exact",exact(c)).put("notifications",notifications(c,REMINDERS)).put("overlay",android.provider.Settings.canDrawOverlays(c)).put("delivery",ledger(c));return out;}
